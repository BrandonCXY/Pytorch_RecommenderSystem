...
class DeepFactorizationMachineModel(torch.nn.Module):
    ...
    对于实现DeepFM模型中如何处理数值型变量的问题可以分为两类：
    （1）预处理dataset，就比如离散化分桶+one hot+embedding 
    然后当做categorical的来处理；

    （2）知乎的解答：当特征X并没有专门one-hot编码，包含类别和数值变量时，对于输入数据，获取到feature_index, feature_value格式，
    此时先通过比如embeddinglookup的方式，获取到隐向量W矩阵中对于的向量组w, 然后w * value，因此第2中特征场景下，
    相比第1种离散化后的情况，多了一步w与feature_value乘积操作（在特征离散化情况下，value为0，1两种值，因此无需再进行w*value操作）
    """
...
dataset = CriteoDataset('/home/dm/Downloads/Kaggle_Criteo/train_sub1m.txt')
loader = DataLoader(dataset, batch_size=56, num_workers=8)
for i ,(a,b) in enumerate(loader):
    x = a 
    break
